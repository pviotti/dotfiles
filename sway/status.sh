...
# This file uses Noto Emoji font for the icons.
...
# Column number 5 contains the current volume percentage in brackets, e.g.,
...
audio_volume=$(amixer -M get Master |
    awk '/Front Left:/ {print $6=="[off]" ? $5" 🔇":  $5" 🔉"}' |\
    tr -d [])

# Keyboard layout
current_kb_layout=$(swaymsg -p -t get_inputs |
    grep "Microsoft Wired Keyboard 600$" -A5 -m1 | tr -d "[:space:]" | cut -d":" -f9)
